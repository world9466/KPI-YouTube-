...
def video_id_search(program):
    ...
    # 日期篩選功能
    '''
    video_data_id = video_data[['video_id','time_published']][(video_data['video_title'].str.contains('{}'.format(program))) & (video_data['time_published'] >= day_start) & (video_data['time_published'] <= day_end)]
    '''
...
# 輸出excel檔
day_start = re.sub('/','',day_start)
day_end = re.sub('/','',day_end)
id_table.to_excel('../video_id.xlsx',encoding = 'utf-8-sig')
